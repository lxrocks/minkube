...
### This Vagranfile has been tested on ubuntu host
### but should work on Mac without too many tweaks
### Install vagrant and Virtualbox then run
### vagrant up 
### This will build 4 nodes.  control , worker01, worker02 etc
### The login user is vagrant password vagrant
### Once the control host is up 
### run sudo kubeadm init
### instll CIDR driver
### take not of the token and kubeadm join command 
### on the workers
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "worker01" do |config|
    ...
    config.vm.box_version = "2004.01"
    config.vm.box_check_update = false
    config.vm.network :private_network, ip: "10.8.8.11"
...
##### DEFINE worker02 #####
  config.vm.define "worker02" do |config|
    ...
    config.vm.box_version = "2004.01"
    config.vm.box_check_update = false
    config.vm.network :private_network, ip: "10.8.8.12"
...
##### DEFINE worker02 #####
  config.vm.define "worker03" do |config|
    ...
    config.vm.network :private_network, ip: "10.8.8.13"
